...
func RoutePatternMatch(path, pattern string) bool {
	...
	patternPretty = futils.ToLower(patternPretty)
	path = futils.ToLower(path)
	// Strict routing, remove trailing slashes
	if len(patternPretty) > 1 {
		patternPretty = futils.TrimRight(patternPretty, '/')
	}
	...
	for i := segLen - 1; i >= 0; i-- {
		...
		if segs[i].IsParam {
			...
			if len(comparePart) > 1 {
				comparePart = futils.TrimRight(comparePart, slashDelimiterStr)
			}
		}
	}
